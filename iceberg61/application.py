...
def index():
    ...
    if request.method == "GET":
        ...
        # Checks the time diiference is less than 30 min, if not delete the table
        ...
        print(db.execute("SELECT tab FROM users WHERE id = :id", id=session["user_id"]))
...
def buy():
    ...
    if request.method == "POST":
        # Get tab and shares from form input
    ...
    # If the method is GET, return buy page
    else:
...
def check():
    ...
    # Make sure that the user input username
...
def register():
    ...
    if request.method == "POST":
        ...
        if rows:
            return apology("Sorry, this uername is already taken!")
        ...
        #Generate a random number and put into the database
        ...
        # Generate random 4-digit confirmation number
        ...
        # Put all of that into verify afterwards
        # Insert new user info in database
        insert = db.execute("INSERT INTO users (username, hash, fullname) VALUES (:username,:hash,:fullname)",
                            username=request.form.get("username").lower(), hash=generate_password_hash(request.form.get("password")), fullname=request.form.get("fullname"))
    ...
    if request.method == "POST":
        ...
        if info != [] and str(request.form.get("code")) == str(info[0]["code"]):
            ...
            insert = db.execute("INSERT INTO users (username, hash, fullname, email) VALUES (:username,:hash, :fullname, :email)",
                                username=info[0]["username"].lower(), hash=info[0]["hash"], fullname=info[0]["fullname"], email=e)
...
def sell():
    """Add friend"""
    if request.method == "POST":
        # Get table and shares from form input
        ...
        other = db.execute("INSERT INTO friends (userid1, userid2) VALUES (:userid1,:userid2)",
                           userid1=session["user_id"], userid2=friendid[0]["id"])

        return redirect("/")

    else:
        # Pass summed stocks to the html file
        #sumStocks = db.execute(
        #    "SELECT SUM(shares) as totalShares, SUM(total) as totalCash, * FROM transactions WHERE userid=:userid GROUP BY tab", userid=session["user_id"])
...
def change():
    ...
    if request.method == "POST":
        ...
        x = db.execute("UPDATE users SET hash = :hash WHERE id = :userID", userID=session["user_id"], hash=generate_password_hash(request.form.get("password")))
        return redirect("/")
